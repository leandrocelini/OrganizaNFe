...
Function ConfirmaTransacao(Imprimir:Boolean=False):Boolean;
   ...
   For iTentativas := 1 to 7 do begin
      ...
      If ( iTentativas = 7 ) then begin
         ...
         If TEF.AbreGerenciador Then begin
            ...
            End;
         End else begin
            MessageDlg('Erro de Comunicação - Client SiTef.' + #13 +
                       'Favor Ativa-lo!!!',
                       mtInformation,[mbOk],0);
...
Var cConteudoArquivo, cLinhaArquivo, cLinha,
    cCampoArquivo, cValorPago, cValorSaque, cErro, cTipoTransacao: string;
    cArquivo  : TextFile;
    lFlag     : longbool;
...
begin
         ...
         Try
            ...
            while not EOF( cArquivo ) do begin
               ...
               case StrToIntDef( cCampoArquivo, -1 ) of
                  ...
                  200:Begin
                     cValorSaque := Copy(cLinhaArquivo,11,Length(cLinhaArquivo)-10);
                  End;
               End;
            End;
         Except
   ...
   If ( Result = 1 ) then begin
      // Neste ponto é criado o arquivo PENDENTE.TXT, indicando que há uma
      ...
      DadosTransacoes[iConta].ValorSaque     := StrToFloatDef(cValorSaque,0)/100;
   End else begin
      If ( FileExists( TEF.DirResp + '\INTPOS.001' ) ) then
         DeleteFile( TEF.DirResp + '\INTPOS.001' );
...
begin
   ...
   Try
      ...
      DadosComprovante.Valor       := DadosTransacoes[iConta].Valor+
                                      DadosTransacoes[iConta].ValorSaque;
...
Function ConfirmaTransacao(Imprimir:Boolean=False): boolean;
////////////////////////////////////////////////////////////
Var cLinhaArquivo, cConteudo, cNomeArquivo, cLinha, cNSU, cNSUAux,
    cTransacao, cTipo:String;
    iTentativas, iConta, n:Integer;
    cArquivo:TextFile;
    lFlag:longbool;
begin
   cTransacao := '';
   ...
   //Confirma todas transações pendentes
   For n := 1 to iConta do begin
      cLinhaArquivo := '';
      cConteudo     := '';
      cNSU          := '';
      lFlag         := False;
      cNomeArquivo  := TEF.DirTmp + '\INTPOS' + IntToStr( n ) + '.001';
      ...
      If FileExists( cNomeArquivo ) then begin
         ...
         while not EOF( cArquivo ) do begin
            ReadLn( cArquivo, cLinhaArquivo );
            If ( copy( cLinhaArquivo, 1, 3 ) = '000' ) Then begin
               cTipo := Trim(copy( cLinhaArquivo, 11, Length( cLinhaArquivo ) - 10 ));
            End else If ( copy( cLinhaArquivo, 1, 3 ) = '001' ) or
            ...
            End else If ( copy( cLinhaArquivo, 1, 3 ) = '009' ) Then begin
               cTransacao := Trim(copy( cLinhaArquivo, 11, Length( cLinhaArquivo ) - 10 ));
            End else If ( copy( cLinhaArquivo, 1, 3 ) = '012' ) Then begin
               cConteudo := cConteudo + cLinhaArquivo + #13 + #10;
               cNSU := Trim(copy( cLinhaArquivo, 11, Length( cLinhaArquivo ) - 10 ));
            End;
         ...
         If ( ( cNSU <> '' ) And ( cTipo = 'ADM' ) ) Or
            ( ( cTransacao = '0' ) And ( ( cTipo = 'CRT' ) Or ( cTipo = 'CHQ' ) ) ) Then begin
         ...
         //Apaga o arquivo da transação
         If ( FileExists( cNomeArquivo ) ) then begin
            DeleteFile( cNomeArquivo );
         End;

         cNSUAux := cNSUAux + cNSU + #13;

         DeleteFile( 'IMPRIME' + IntToStr(n) + '.TXT' );

         //Retorna verdadeiro se houve confirmação de alguma transação
         Result := True;
      End;
   End;
   ...
   If Imprimir And ( ( cTransacao = '0' ) And ( cTipo = 'CRT' ) )Then begin
      //Limpa o buffer do teclado e do mouse para mostrar a msg
      mbFuncoes.EmptyKeyQueue();
      mbFuncoes.EmptyMouseQueue();
      Application.ProcessMessages;
      MessageDlg('Última transação foi confirmada.' + #13 +
                 'NSU: ' + cNSUAux,mtInformation,[mbOk],0);
   End;
end;
...
Function NaoConfirmaTransacao():Boolean;
////////////////////////////////////////
var cLinhaArquivo, cConteudo, cCampoArquivo, cNomeArquivo, cErro: string;
...
begin
   ...
   For n := 1 to iConta do begin
      ...
      If FileExists( cNomeArquivo ) then begin
         ...
         //Verifica se tem numero de transação se não houver não manda o NCN
         If ( cNSU <> '' ) Then begin
         ...
         //Apaga o arquivo com a imagem do cupom do TEF
         DeleteFile( 'IMPRIME' + IntToStr(n) + '.TXT' );

         //Aguarda 2 segundos o sistema operacional apagar o arquivo
         Sleep(1000);
         ...
         //Retorna verdadeiro se houve transação cancelada
         Result := True;
      End;
   End;
   ...
   mbFuncoes.EmptyMouseQueue();
   Application.ProcessMessages;
   MessageDlg('Transação não efetuada. Favor reter o Cupom.',
              mtInformation,[mbOk],0);
end;
...
/////////////////////////////////////////////////////////////////
begin
   //TEF dedicado não tem essa opção pois as transações podem ser
   //confirmadas no final de todas as impressões
end;
